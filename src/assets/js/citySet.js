import record from './cityJson'
import area from './cityJson'
import Iput from './Popt'
var cityMethod = {
  SelCity: function (obj, e) {
    ...
    var tb_province = []
    var b = record.province
    for (var i = 0, len = b.length; i < len; i++) {
      tb_province.push('<a data-level="0" data-id="' + b[i]['id'] + '" data-name="' + b[i]['name'] + '">' + b[i]['name'] + '</a>')
    }
    $('#_citys0').append(tb_province.join(''))
    $('#_citys0 a').click(function () {
      var g = that.getCity($(this))
      $('#_citys1 a').remove()
      $('#_citys1').append(g)
      $('._citys1').hide()
      $('._citys1:eq(1)').show()
